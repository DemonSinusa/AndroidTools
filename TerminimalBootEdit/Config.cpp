...
Config::Config() {
	...
	memset(&it_cfg, 0x00, sizeof (onegen_max_bootheader));
}
...
int Config::EatBinConfig(void *data, unsigned int len) {
	int parlen = 0;
	onegen_max_bootheader *curboot = NULL;
	if (len >= sizeof (boot_img_hdr))
		curboot = (onegen_max_bootheader *) data;
	else
		return 0;
	...
	sprintf(options[PROP_BOOT_VERSION].txtname,"%d",curboot->header_version);
	options[PROP_BOOT_VERSION].len=sizeof (uint32_t);
	SetType(&options[PROP_BOOT_VERSION],PROP_BOOT_VERSION);
...
//Далее версия 1 заголовка
	if(curboot->header_version>0){
		...
		if (curboot->recovery_dtbo_offset>0) {
			...
			options[PROP_DBO_ADDR].len = sizeof (uint64_t);
			SetType(&options[PROP_DBO_ADDR], PROP_DBO_ADDR);
		}

//Далее версия 2 заголовка
		if(curboot->header_version>1){
			if(curboot->dtb_addr>0){
				sprintf(options[PROP_DTB_ADDR].txtname,"0x%16lx",curboot->dtb_addr);
				options[PROP_DTB_ADDR].len=sizeof(uint64_t);
				SetType(&options[PROP_DTB_ADDR],PROP_DTB_ADDR);
			}
		}
	}
...
onegen_max_bootheader *Config::GetHeader() {
	memset(&it_cfg, 0x00, sizeof (onegen_max_bootheader));

	if (options[PROP_BOOT_VERSION].len > 0)
		sscanf(options[PROP_BOOT_VERSION].txtname, "%d", &it_cfg.header_version);
	...
	if(options[PROP_DTB_ADDR].len>0)
		sscanf(options[PROP_DTB_ADDR].txtname,"0x%16lx",&it_cfg.dtb_addr);
